...
from gcp_pricing import GCP_PRICING, TEAM_GCP_USAGE
...
def get_ri_coverage(date, start_date, resource_type):
    days = (date - start_date).days
    if resource_type not in ['n2-standard-2', 'n2-standard-4', 'cloudsql_medium', 'cloudsql_large']:
        return 1.0
    if days < 365:
        ...
        for team in teams:
            ...
            for resource_type in resources:
                ...
                if resource_type in ['e2-micro', 'e2-small', 'e2-medium', 'n2-standard-2', 'n2-standard-4', 'a2-highgpu-1g']:
                    ...
                    unit_cost = GCP_PRICING[resource_type] / 30
                elif resource_type.startswith('cloudsql'):
                    service = 'CloudSQL'
                    unit = 'Hours'
                ...
                records.append({
                    'Date': date, 'Cloud': 'GCP', 'Service': service, 'ResourceType': resource_type,
...
print("Generating GCP costs...")
df_gcp = generate_gcp_costs(date_range, teams)
...
for date in date_range:
    ...
    tagging_status, standard_version = get_tagging_status(date, start_date)
    for team in teams:
        transfer_gb = np.random.uniform(350, 1000) if team == 'DataScience' else np.random.uniform(30, 150)
        if (date - start_date).days >= 545:
            transfer_gb *= 0.88
        transfer_cost = transfer_gb * GCP_PRICING['egress'] * BASE_SCALE * monthly_factor
        ...
        transfer_records.append({
            'Date': date, 'Cloud': 'GCP', 'Service': 'DataTransfer', 'ResourceType': 'Egress',
            'Team': team_tag, 'Environment': env_tag, 'CostCenter': costcenter_tag,
            'Usage': round(transfer_gb, 2), 'Unit': 'GB', 'UnitCost': GCP_PRICING['egress'],
...
df_gcp = pd.concat([df_gcp, df_transfer], ignore_index=True)

output_path = '../data/gcp_costs.csv'
df_gcp.to_csv(output_path, index=False)

df_gcp['Date'] = pd.to_datetime(df_gcp['Date'])
year_2023 = df_gcp[df_gcp['Date'].dt.year == 2023]
year_2024 = df_gcp[df_gcp['Date'].dt.year == 2024]
